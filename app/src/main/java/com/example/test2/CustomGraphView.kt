    ...
    private fun initView(context: Context, attrs: AttributeSet?) {
        ...
        if (attrs != null) {
            val a = context.theme.obtainStyledAttributes(
                    attrs, R.styleable.CustomGraphView, 0, 0
            )

            try {
                calculatedStrokeWidth = a.getDimensionPixelSize(R.styleable.CustomGraphView_strokeWidth, 13).toFloat()
                textSize = a.getDimensionPixelSize(R.styleable.CustomGraphView_textSize, 0).toFloat()
            } finally {
                a.recycle()
            }
        }
